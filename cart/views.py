...
# trong app cart file vnpay import class vnpay
...
def checkout(request):
    ...
    if request.POST.get('btnOrder'):
        ...
        # cập nhật version khi nhận được code từ vnpay
        vnp.requestData['vnp_Command'] = 'pay'
        vnp.requestData['vnp_TmnCode'] = settings.VNPAY_TMN_CODE
        vnp.requestData['vnp_Amount'] = int(order.total)*100
        # *100 quy định của VNPAY
        ...
        vnp.requestData['vnp_CreateDate'] = datetime.now().strftime('%Y%m%d%H%M%S') #Định dạng ngày giờ theo VNPAY
...
def success(request):
    ...
    if inputData:
        ...
        amount = int(inputData['vnp_Amount'])/100 #Chia do dữ liệu phía trên đã nhân cho 100, interger do lấy dữ liệu từ dictionary về
